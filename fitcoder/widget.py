...
from .stats import format_duration, stats_table, stats_totals


def tk_is_available() -> bool:
    return tk is not None and ttk is not None and tkfont is not None
...
class FitCoderWidgetApp:
    ...
    def _stats_summary_text(self) -> str:
        ...
        total_reps, total_rounds = stats_totals(self.exercises)
        self.total_reps_var.set(str(total_reps))
        self.total_rounds_var.set(str(total_rounds))
        table = stats_table(self.exercises)
        return "\n".join(
            [
